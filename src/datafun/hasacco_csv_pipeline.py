...
Date: 2026-04
  ...
  Paths (relative to repo root):
    ...
    INPUT FILE:  data/raw/math_STAAR_scores.csv
    OUTPUT FILE: data/processed/csv_math_STAAR_stats.txt

  Terminal command to run this file from the root project folder:

    uv run python -m datafun.hasacco_csv_pipeline
...
def load_stats_report(*, stats: dict[str, float], out_path: Path) -> None:
    ...
    with out_path.open("w", encoding="utf-8") as f:
        f.write("CSV Math STAAR Score Statistics\n")
...
def run_csv_pipeline(*, raw_dir: Path, processed_dir: Path, logger: Any) -> None:
    ...
    input_file = raw_dir / "math_STAAR_scores.csv"
    output_file = processed_dir / "csv_math_STAAR_stats.txt"

    # E: Read raw data.
    scores = extract_csv_scores(file_path=input_file, column_name="9M")
