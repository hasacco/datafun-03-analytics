...
Date: 2026-04
  ...
  Paths (relative to repo root):
    ...
    INPUT FILE:  data/raw/maternal_mortality_US.txt
    OUTPUT FILE: data/processed/maternal_mortality_summary.txt

  Terminal command to run this file from the root project folder:

    uv run python -m datafun.hasacco_text_pipeline
...
def run_text_pipeline(*, raw_dir: Path, processed_dir: Path, logger: Any) -> None:
    ...
    input_file = raw_dir / "maternal_mortality_US.txt"
    output_file = processed_dir / "maternal_mortality_summary.txt"
